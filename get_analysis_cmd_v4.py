...
def print_cmd(panel,analysis_fold,run_fold,log):
	if 'K00' in run_fold:
		print ("nohup Autorun_GSCAP_new_0823-2.0.py -p %s %s/20%s_RT > %s 2>&1 & " % (panel,analysis_fold,run_fold,log))
	if 'E00' in run_fold:
		print ("nohup Autorun_GSCAP_new_0823-2.0.py -p %s.X %s/20%s_RT > %s 2>&1 & " % (panel,analysis_fold,run_fold,log))
if len(list_RT) >= 1:
	panel_sample.append(list_RT[0])
	write_id_list(list_RT,"_RT")
	print_cmd("201801-425-plutRT",analysis_fold,run_fold,"RT.log")
if len(list_416) >= 1:
	panel_sample.append(list_416[0])
	write_id_list(list_416,"_416")
	print_cmd("201601",analysis_fold,run_fold,"416.log")
if len(list_422) >= 1:
	panel_sample.append(list_422[0])
	write_id_list(list_422,"_422")
	print_cmd("201702",analysis_fold,run_fold,"422.log")
if len(list_425) >= 1:
...
if len(list_RNA) >= 1:
	...
	print_cmd("leukemia2017",analysis_fold,run_fold,"RNA.log")
if len(list_14gene) >= 1:
	panel_sample.append(list_14gene[0])
	write_id_list(list_14gene,"_14gene")
	print_cmd("14Gene",analysis_fold,run_fold,"14gene.log")
if len(list_YH105) >= 1:
	panel_sample.append(list_YH105[0])
	write_id_list(list_YH105,"_leukemia")
	print_cmd("leukemia2017",analysis_fold,run_fold,"leukemia.log")
if len(list_Lu) >= 1:
	panel_sample.append(list_Lu[0])
	write_id_list(list_Lu,"_Lu")
	print_cmd("LungFullFinal",analysis_fold,run_fold,"Lu.log")
